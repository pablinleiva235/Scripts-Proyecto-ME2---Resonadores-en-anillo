...
# --- CARGAR ARCHIVOS S2P ---
try:
    ntwk_rogers = rf.Network('Rogers_2GHz_puntos.s2p')
except FileNotFoundError:
    print("ERROR: No se encontró 'Rogers_2GHz_puntos.s2p'.")
    exit()

try:
    ntwk_fr4 = rf.Network('FR4_2GHz_puntos.s2p')
except FileNotFoundError:
...
ntwk_rogers.s21.plot_s_db(ax=ax[0], label='$|S_{21}|$ (S21 Rogers 2GHZ)')
...
ntwk_fr4.s21.plot_s_db(ax=ax[1],label='$|S_{21}|$ (S21 FR4 2GHZ)')
...
# Filtrar hasta 6.5 GHz
mask = frecuencias_rogers_GHz <= 6.5
...
plt.ylabel('εr(f)')
plt.title('Cálculo de Permitividad Relativa (εr) para 2GHZ: ROGERS 4350B vs FR4')
